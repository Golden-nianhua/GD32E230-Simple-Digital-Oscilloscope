...
{
    uint16_t i=0,j=0;
    TFT_Address_Set16(xsta,ysta,xend-1,yend-1);   //设置显示范围
    for(i=ysta;i<yend;i++)
    {
        for(j=xsta;j<xend;j++)
        {
...
{
    TFT_Address_Set16(x,y,x,y);   //设置显示范围
    TFT_WR_DATA16(color);
}
...
void drawCurve(uint8_t yOffset,short int rawValue)  
{
	...
	{
		TFT_DrawPoint(0,y,YELLOW);  //画线改成黄色
//		TFT_DrawPoint(0,y,GREEN); 		
	...
	else
	{
		...
		if(x<100)  //不超过屏幕宽度
		{
			TFT_DrawLine(lastX,lastY,x,y,YELLOW);
...
void TFT_ShowChar(uint16_t x,uint16_t y,uint8_t num,uint16_t fc,uint16_t bc,uint8_t sizey,uint8_t mode)
{
	...
	num=num-' ';    //得到偏移后的值
	TFT_Address_Set16(x,y,x+sizex-1,y+sizey-1);  //设置光标位置 
...
void TFT_StaticUI(void)
{
    ...
    TFT_ShowChinese(10,0,(uint8_t *)"立创训练营",RED,BLACK,16,0);  //黑底红字
...
void TFT_ShowUI(volatile const struct Oscilloscope *value)
{
    ...
    TFT_ShowString(5,106,(uint8_t *)showData,BLACK,GREEN,16,0);   //绿色字显示电压幅值
    memset(showData,0,32);
    
    if((*value).gatherFreq>=1000)
    {
        sprintf(showData,"%2.0fKHz",(*value).gatherFreq/1000.0f);    //绿色字显示频率 KHz
    ...
    else
    {
        ...
        TFT_ShowString(55,106,(uint8_t *)showData,BLACK,GREEN,16,0);  //绿色字显示频率 Hz
        memset(showData,0,32);  
    }
    ...
    {
        TFT_ShowChinese(118,36,(uint8_t *)"关闭",WHITE,BLACK,16,0);     //关闭时改成黑底白字
//        TFT_ShowChinese(118,36,(uint8_t *)"关闭",BLACK,YELLOW,16,0);   		
    ...
    sprintf(showData,"%3.1f%%  ",(((*value).pwmOut)/((*value).timerPeriod+0.0f))*100);  //显示占空比
    TFT_ShowString(110,106,(uint8_t *)showData,BLACK,YELLOW,16,0);  
    memset(showData,0,32);        
}
